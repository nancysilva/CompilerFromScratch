...
public class Scanner {
	...
	static Scanner() {
		...
		for (int i = 65; i <= 90; ++i) start[i] = 28;
		for (int i = 95; i <= 95; ++i) start[i] = 28;
		for (int i = 97; i <= 122; ++i) start[i] = 28;
		start[123] = 1; 
		start[125] = 2; 
		start[40] = 3; 
		start[41] = 4; 
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 26:
				if (ch <= 9 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= '!' || ch >= '#' && ch <= '[' || ch >= ']' && ch <= 65535) {AddCh(); goto case 26;}
			...
			case 28:
				...
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 28;}
				else {t.kind = 30; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 29:
